...
const { verifiedDoctorService } = require('.');
// const ApiError = require('../utils/ApiError');

const acceptDoctorVerification = async (doctorauthid, AuthData) => {
  const doctorVerifiedData = await verifiedDoctorService.createVerifiedDoctor(doctorauthid, AuthData._id);
...
const fetchDoctorProfile = async (id) => {
  const basicDetails = await DoctorBasic.find({ auth: `${id}` });
  ...
  const res = { fullname, basicDetails, documentDetails };
  return res;
};
